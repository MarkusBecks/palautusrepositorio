describe('Blog app', function () {
  ...
  describe('When logged in', function () {
    ...
    it('Blogs are sorted by likes', function () {
      // create 3 blogs
      cy.createBlog({ title: 'Blog with the most likes', author: 'Test Author', url: 'testurl' })
      cy.createBlog({ title: 'Blog with the second most likes', author: 'Test Author', url: 'testurl' })
      cy.createBlog({ title: 'Blog with the least likes', author: 'Test Author', url: 'testurl' })
      // view all the blogs to gain access to like button
      cy.get('.viewButton').contains('view').click()
      cy.get('.viewButton').contains('view').click()
      cy.get('.viewButton').contains('view').click()
      // click like buttons
      cy.contains('Blog with the most likes').parent().find('.likesButton').click().click().click()
      cy.contains('Blog with the second most likes').parent().find('.likesButton').click().click()
      cy.contains('Blog with the least likes').parent().find('.likesButton').click()
